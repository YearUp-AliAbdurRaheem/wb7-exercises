...
# 1. How many suppliers are there? Use a query!
# --------------------------------------------- #
SELECT COUNT(*) AS NumberOfSuppliers FROM Suppliers;
...
# 3. What is the price of the cheapest item that Northwind sells?
# --------------------------------------------------------------- #
SELECT MIN(UnitPrice) AS CheapestItemPrice FROM Products;

# ----------------------------------------------------------- #
# 4. What is the average price of items that Northwind sells?
# ----------------------------------------------------------- #
SELECT AVG(UnitPrice) AS AverageItemPrice FROM Products;

# --------------------------------------------------------------------- #
# 5. What is the price of the most expensive item that Northwind sells?
# --------------------------------------------------------------------- #
SELECT MAX(UnitPrice) AS MostExpensiveItemPrice FROM Products;
...
SELECT 
    SupplierID
    , COUNT(SupplierID) as ItemSupplied
FROM Products
GROUP BY SupplierID;
